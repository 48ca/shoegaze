...
var loopRain = function() {
	...
	var cx = parseFloat(drop.getAttribute('cx'));
	    ...
	    if(cx < width/2) {
		drop.setAttribute("cy", cy-vy*(velocity_contribution + velocity_contribution * Math.abs(cy-height/2)/(height/2)));
	    	drop.setAttribute("cx", cx-vx*(velocity_contribution + velocity_contribution * Math.abs(cx-width/2)/(width/2)));
	    } else {
            	drop.setAttribute("cy", cy+vy*(velocity_contribution + velocity_contribution * Math.abs(cy-height/2)/(height/2)));
	    	drop.setAttribute("cx", cx+vx*(velocity_contribution + velocity_contribution * Math.abs(cx-width/2)/(width/2)));
	    }
